...
struct TerminalVisibilityDetector {
    ...
    /// Check if an OpenCode session is currently focused (user is looking at it)
    /// - Parameter sessionPid: The PID of the OpenCode process
    ...
    static func isSessionFocused(sessionPid: Int) async -> Bool {
        ...
        if isInTmux {
            ...
            return await TmuxTargetFinder.shared.isSessionPaneActive(opencodePid: sessionPid)
        } else {
