...
//  OpenCodeIsland
//
//  Finds tmux targets for OpenCode processes
//

import Foundation

/// Finds tmux session/window/pane targets for OpenCode processes
actor TmuxTargetFinder {
    static let shared = TmuxTargetFinder()

    private init() {}

    /// Find the tmux target for a given OpenCode PID
    func findTarget(forOpenCodePid openCodePid: Int) async -> TmuxTarget? {
        ...
        for line in output.components(separatedBy: "\n") {
            ...
            if ProcessTreeBuilder.shared.isDescendant(targetPid: openCodePid, ofAncestor: panePid, tree: tree) {
    ...
    func isSessionPaneActive(opencodePid: Int) async -> Bool {
        guard let tmuxPath = await TmuxPathFinder.shared.getTmuxPath() else {
            return false
        }

        // Find which pane the OpenCode session is in
        guard let sessionTarget = await findTarget(forOpenCodePid: opencodePid) else {
            return false
        }
