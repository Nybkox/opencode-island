...
actor YabaiController {
    ...
    /// Focus the terminal window for a given OpenCode PID (tmux only)
    func focusWindow(forOpenCodePid openCodePid: Int) async -> Bool {
        ...
        return await focusTmuxInstance(openCodePid: openCodePid, tree: tree, windows: windows)
    }
    ...
    private func focusTmuxInstance(openCodePid: Int, tree: [Int: ProcessInfo], windows: [YabaiWindow]) async -> Bool {
        // Find the tmux target for this OpenCode process
        guard let target = await TmuxController.shared.findTmuxTarget(forOpenCodePid: openCodePid) else {
            return false
        }
    ...
    private func focusTmuxPane(forWorkingDir workingDir: String, tree: [Int: ProcessInfo], windows: [YabaiWindow]) async -> Bool {
        ...
        do {
            ...
            for pane in panes {
                ...
                // Check if this pane has a OpenCode child with matching cwd
                for (pid, info) in tree {
                    let isChild = ProcessTreeBuilder.shared.isDescendant(targetPid: pid, ofAncestor: panePid, tree: tree)
                    let isOpenCode = info.command.lowercased().contains("opencode")

                    guard isChild, isOpenCode else { continue }
