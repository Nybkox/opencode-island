...
private let logger = Logger(subsystem: "com.opencodeisland", category: "Hooks")

/// Event received from OpenCode hooks
struct HookEvent: Codable, Sendable {
    ...
    init(
        ...
        tool: String?, toolInput: [String: AnyCodable]?, toolUseId: String?,
        notificationType: String?, message: String?, serverPort: Int? = 4096
    ) {
...
/// Unix domain socket server that receives events from OpenCode hooks
...
class HookSocketServer {
    ...
    /// Cancel all pending permissions for a session (when OpenCode stops waiting)
    ...
    func getPendingPermission(sessionId: String) -> (
        ...
        permissionsLock.lock()
        defer { permissionsLock.unlock() }
        guard let pending = pendingPermissions.values.first(where: { $0.sessionId == sessionId })
        else {
            return nil
        }
