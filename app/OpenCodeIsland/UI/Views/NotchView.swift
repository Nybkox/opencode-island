...
struct NotchView: View {
    ...
    @StateObject private var sessionMonitor = OpenCodeSessionMonitor()
    ...
    /// Whether any OpenCode session is currently processing or compacting
    private var isAnyProcessing: Bool {
        sessionMonitor.instances.contains { $0.phase == .processing || $0.phase == .compacting }
    }

    /// Whether any OpenCode session has a pending permission request
    ...
    private var isProcessing: Bool {
        activityCoordinator.expandingActivity.show && activityCoordinator.expandingActivity.type == .opencode
    }
    ...
    private var headerRow: some View {
        HStack(spacing: 0) {
            ...
            if showClosedActivity {
                HStack(spacing: 4) {
                    OpenCodeCrabIcon(size: 14, animateLegs: isProcessing)
            ...
            Button {
                ...
                ZStack(alignment: .topTrailing) {
                    Image(systemName: viewModel.contentType == .menu ? "xmark" : "line.3.horizontal")
                        ...
                        .foregroundColor(TerminalColors.icon)
    ...
    private func handleProcessingChange() {
        if isAnyProcessing || hasPendingPermission {
            // Show opencode activity when processing or waiting for permission
            activityCoordinator.showActivity(type: .opencode)
            isVisible = true
        } else {
