...
private let logger = Logger(subsystem: "com.opencodeisland", category: "ChatView")

struct ChatView: View {
    let sessionId: String
    let initialSession: SessionState
    let sessionMonitor: OpenCodeSessionMonitor
    ...
    init(
        sessionId: String, initialSession: SessionState, sessionMonitor: OpenCodeSessionMonitor,
    ...
    var body: some View {
        ...
        .task {
            ...
            // History is loaded from backend via BackendClient.$messages -> ChatHistoryManager
        ...
        .onReceive(ChatHistoryManager.shared.$histories) { _ in
            // Use history(for:) which handles truncated vs full session ID matching
            let newHistory = ChatHistoryManager.shared.history(for: sessionId)

            logger.info("ChatView onReceive: sessionId=\(sessionId), newHistory.count=\(newHistory.count), current history.count=\(history.count)")
            ...
            if countChanged || lastItemChanged || newHistory != history {
                logger.info("ChatView updating history: \(newHistory.count) items")
            ...
            } else if hasLoadedOnce && newHistory.isEmpty && !history.isEmpty {
    ...
    private var messageList: some View {
        ScrollViewReader { proxy in
            ...
            .animation(
                ...
                value: isAutoscrollPaused && newMessageCount > 0)
        }
    }
    ...
    private var inputBar: some View {
        HStack(spacing: 10) {
            TextField(
                canSendMessages
                    ? "Message OpenCode..." : "Open OpenCode in tmux to enable messaging",
                text: $inputText
            )
    ...
    private func focusTerminal() {
        Task {
            if let pid = session.pid {
                _ = await YabaiController.shared.focusWindow(forOpenCodePid: pid)
            } else {
...
struct ToolCallView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            if showContent && tool.status != .running && tool.name != "Task"
...
struct NewMessagesIndicator: View {
    ...
    var body: some View {
        Button(action: onTap) {
            ...
            .background(
                Capsule()
                    .fill(Color(red: 0.85, green: 0.47, blue: 0.34))  // OpenCode orange
